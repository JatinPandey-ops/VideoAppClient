...
function ChannelPage() {
  ...
  console.log(videos)
  const defaultImg =
    "https://firebasestorage.googleapis.com/v0/b/videoapp-92f3c.appspot.com/o/Cover%2Fpexels-ben-cheung-715134.jpg?alt=media&token=e1bfa83a-7c7f-442d-b43d-aed6f00e1fe0";
  ...
  useEffect(() => {
    const fetchUserVideos = async () => {
      ...
      const res = await axios.get(`video/uservideo/${path}`);
      setVideos(res.data);
  ...
  const handleSubscribe = async () => {
    if (currentUser.subscribedUsers?.includes(data._id)) {
      await axios.put(`users/unsub/${currentChannel._id}`, undefined, {
        withCredentials: true,
      });
      dispatch(subscription(currentChannel._id));
    } else {
      await axios.put(`users/sub/${currentChannel._id}`, undefined, {
        withCredentials: true,
      });
      dispatch(subscription(currentChannel._id));
    }
    ...
    <>
      {loading === true ? (
        ...
        <Box>
            ...
            <Stack direction="row" justifyContent="space-between">
              <Box>
                <Stack alignItems="center">
                  ...
                  <Stack marginTop="5px" alignItems="center">
          ...
          <Box
            padding="20px 5px"
            sx={{ display: owner === true ? "block" : "none" }}
          >
          ...
          <Box>
            {videos.length === 0 ? (
              <BlankPage message="No videos Found" />
            ) : (
              <Container>
                {loadingVideo === true ? (
                  <Loading />
